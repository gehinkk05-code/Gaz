...
function ImGui:CreateWindow(WindowConfig)
	...
	--// Сайдбар — ScrollingFrame для поддержки скролла
	...
	SideBar.ZIndex = 2
	SideBar.ScrollBarThickness = 0 -- скрываем полосу прокрутки
	SideBar.ScrollingDirection = Enum.ScrollingDirection.Y
	SideBar.CanvasSize = UDim2.new(0, 0, 0, 0) -- автоматически расширяется
	...
	function WindowConfig:UpdateBody()
		...
		--// Body занимает всё окно минус titlebar по высоте
		Body.Size = UDim2.new(1, 0, 1, -HeaderSizeY)
		Body.Position = UDim2.new(0, 0, 0, HeaderSizeY)
	end
	...
	function WindowConfig:CreateTab(Config)
		...
		Content.Name = Name
		Content.Size = UDim2.new(1, -SideWidth, 1, 0)       -- ширина минус сайдбар
		Content.Position = UDim2.new(0, SideWidth, 0, 0)    -- смещение вправо на ширину сайдбара
		Content.Parent = Body
	...
	function WindowConfig:ShowTab(TabClass: SharedTable)
		...
		if not TargetPage.Visible and not TabClass.NoAnimation then
			TargetPage.Position = UDim2.fromOffset(HasTabs and SideBarWidth or 0, 5)
		end

		--// Hide other tabs
		for _, Page in next, Body:GetChildren() do
			if Page == SideBar then continue end -- не трогаем сайдбар
			Page.Visible = Page == TargetPage
		end

		--// Page animation
		ImGui:Tween(TargetPage, {
			Position = UDim2.fromOffset(HasTabs and SideBarWidth or 0, 0)
		})
		return self
	end
